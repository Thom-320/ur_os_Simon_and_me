...
public final class SystemOS implements Runnable{
    ...
    public double calcAvgContextSwitches() {
        ...
        // Count non-idle segments: contiguous runs of the same PID (>=0).
        // A new segment starts when we see the first non-idle after idle, or
        // when the running PID changes.
        int segments = 0;
        Integer last = null; // last non-idle PID; null means idle or start
        for (int i = 0; i < execution.size(); i++) {
            ...
            if (last == null || cur != last.intValue()) {
                segments++;
            }
            last = cur;
        }
